...
func (template *Template) configureFont(text string) (err error) {
	template.TextConfig.FontConfig.Font, err = truetype.Parse(goregular.TTF)
	if err != nil { return }
	template.TextConfig.reachedImageBorder = false
	template.TextConfig.FontConfig.FontSize = 10
	for ;; {
		fontFace := truetype.NewFace(template.TextConfig.FontConfig.Font, &truetype.Options{
		...
		if int(heightText) > (template.PaddingBottom / 2) { template.TextConfig.FontConfig.FontSize -= 1 }
		if int(heightText) < (template.PaddingBottom / 2) { template.TextConfig.FontConfig.FontSize += 1 }
...
func (template *Template) RenderTexts() (err error) {
	...
	fontSizeUpper := template.TextConfig.FontConfig.FontSize
	if fontSizeUpper < 10 { fontSizeUpper = 0 }

	if err = template.configureFont(template.TextConfig.Texts[1]); err != nil { return }
	fontSizeLower := template.TextConfig.FontConfig.FontSize
	if !template.TextConfig.reachedImageBorder { fontSizeLower -= 35 }
	if fontSizeLower < 10 { fontSizeLower = 0 }

	fontFaceUpper := truetype.NewFace(template.TextConfig.FontConfig.Font, &truetype.Options{
		Size: float64(fontSizeUpper),
	})
	fontFaceLower := truetype.NewFace(template.TextConfig.FontConfig.Font, &truetype.Options{
		Size: float64(fontSizeLower),
	})
